        ...
        if isSelecting {
            ...
            .contextMenu {
                Button {
                    ...
                    Label(language.text("common.info"), systemImage: "info.circle")
                }
        ...
        NavigationStack {
            List {
                Section("ITEM") {
                    ...
                    LabeledContent("Type", value: entry.isDirectory ? "Folder" : "File")
                    LabeledContent("Path", value: entry.path)
                }

                Section("SIZE") {
                    HStack {
                        Label("Size", systemImage: entry.isDirectory ? "folder" : "doc")
                        Spacer()
                        if isCalculating {
                            ProgressView()
                        } else {
                    ...
                    if let byteCount {
                        ...
                        Text(entry.isDirectory
                             ? "Calculating the total size of all files inside this folder…"
                             : "Reading file size…")
                            .font(.caption)
                            .foregroundStyle(.secondary)
                    }
            ...
            .task {
                ...
                let size = await Task.detached(priority: .userInitiated) {
                    ContainerStore.itemSize(at: path)
                }.value
                byteCount = size
                isCalculating = false
            }

...
enum ContainerStore {
    ...
    /// Calculates the logical file size of a file or directory recursively.
    /// Directories are intentionally calculated on demand so browsing a large
    /// container does not block while every row is being enumerated.
    static func itemSize(at path: String) -> Int64 {
        ...
        for case let url as URL in enumerator {
            autoreleasepool {
                ...
                total = total.addingReportingOverflow(bytes).overflow ? Int64.max : total + bytes
            }
        }
        return total
    }
